...
def sql_adder(massiv):
    try:
        with connect(
            ...
            selec_time_query = f"""call bot3.add_apply{tuple(massiv)};"""
            with connection.cursor() as cursor:
                cursor.execute(selec_time_query)
                connection.commit()
            return True
        ...
        with connect(
            ...
            for time in timer:
                for name in namer:
                    ...
                    selec_time_query = f"""select bot3.is_excurtion_available('{massiv[0]}', '{massiv[1]}',
...
def addict(message):
    ...
    try:
        ...
        z.add(btn1487, btn1488)
        bot.send_message(message.chat.id, f'Перепроверьте дату{message.text}, всё верно?', reply_markup=z)
    except ValueError:
